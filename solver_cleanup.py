...
class computerBoard(board.board):
    ...
    def primaryClick(self, i, j):
        """
        Overwrite the original method to add an error when you try to click on
        an uncovered tile
        """
        if not self.__getitem__(i, j).covered:
            raise boardError("Tried to click on an uncovered tile {}".format((i, j)))
        else:
            super().primaryClick()
...
class AdvancedSolver(solver):
    ...
    def guess(self):
        ...
        for _ in range(self.board.tileCount):
            guess = self.pickRandom()
            if self.board.isCovered(*guess) and not self.board.isFlag(*guess):
                # should never be necessary so we'll leave this here.
                logging.info("Picking {} at random".format(guess))
                return [guess], 'click'
    ...
    def extendedAreaSearch(self):
        ...
        flagCounter = 0
        # IDEA: flag ALL the tiles that are eligible in one go.
        for i, j in itertools.product(range(self.board.rows), range(self.board.cols)):
            ...
            if self.board.isCovered(i, j):
                ...
                # mark all of the suspicious neighbors as having a numerical
                # suspicion value, rather than None.
        ...
        for i, j in itertools.product(range(self.board.rows), range(self.board.cols)):
            ...
            # Next loop through every possible combination of mines near this
            # tile and record which are potentially valid.
            # keep track of which combinations are good and how many we've
            # analyzed.
            goodCombos = []
            comboCounter = 0
            ...
            for combo in combinations:
                # create a flag for a bad combination.
                # IDEA: use a for-else instead.
                badCombo = False
                comboCounter += 1
                # create a list of the neighbors of the current tile, and also
                # all the neighbors of mines in the current combination.
                neighbors = self.board.getNeighbors(i, j)
                for tile in combo:
                ...
                for (row, col) in neighbors:
                    ...
                    minesNearNeighbor = sum([self.isNear(mine, (row, col)) for mine in combo])
                    ...
                    # near the neighbor in question, abort.
                        ...
                        badCombo = True
                        break
                    # this is the tricky part. If the combo has too few mines
                    # near the neighbor tile, we can rule it out.
                    # Count the number of tiles near the neighbor, but not near
                    # the tile current tile, that might contain mines.
                    ...
                    # if all the mines the combo puts near the neighbor, plus
                    # all the possible mines far away from the neighbor,
                    # are less than the number of the neighbor tile,
                    # this is a bad combo.
        ...
        # keep track of the number of combinations we've analyzed so far.
        comboCounter = 0
        # commpute the number of combinations we have to analyze
        # IDEA: Make it guess at random or if the number of combos is too big.
        numCombos = 1
        ...
        for wholeBoardCombo in itertools.product(*allCombosList):
            comboCounter += 1
            # IDEA: use a tkinter progress bar instead.
            ...
            # if the combo puts too many mines near any tile in focus,
            # it's not valid.
            # or too few.
            badCombo = False
            for (i, j) in tilesInFocus:
        ...
        for tile in clearTiles:
            if not self.board.isCovered(*tile):
                raise RuntimeError("Tried to clear an uncovered tile")
